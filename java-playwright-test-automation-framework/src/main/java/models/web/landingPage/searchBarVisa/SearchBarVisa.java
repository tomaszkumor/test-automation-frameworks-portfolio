...
public class SearchBarVisa extends SearchBarVisaSelectors {
    ...
    private void changeSpecificTravellersGroupCount(PhpTravelsModel phpTravelsModel) {
        ...
        if (actualTravellersCountAsNumber > expectedTravellersCountAsNumber) {
            String arrowSelector = getArrowButtonSelector(Arrow.LEFT);
            ...
            String arrowSelector = getArrowButtonSelector(Arrow.RIGHT);
    ...
    private String getArrowButtonSelector(Arrow arrow) {
        String arrowDirection = switch (arrow) {
            case LEFT -> "remove";
            case RIGHT -> "add";
        };

        return travellersWindowSelector.concat("//span[text() = '" + arrowDirection + "']/..");
    }

    private void closeTravellersWindow() {
        click.leftClick(h1Selector, "h1");
    }

    private void clickOnVisaTypeSelect() {
        String visaTypeSelectSelector = visaTypeValueSelector.concat("/..");
        click.leftClick(visaTypeSelectSelector, "Visa type select");
        log.info("Visa type select has been clicked.");
    }

    private void clickOnProcessingSpeedSelect() {
        String processingSpeedSelectSelector = processingSpeedValueSelector.concat("/..");
        click.leftClick(processingSpeedSelectSelector, "Processing speed select");
        log.info("Processing speed select has been clicked.");
    }

    private void clickOnTravellersSelect() {
        String travellersSelectSelector = travellersValueSelector.concat("/..");
        click.leftClick(travellersSelectSelector, "Travellers select");
    ...
    private void selectSpecificVisaType(PhpTravelsModel phpTravelsModel) {
        ...
        String specificVisaTypeSelector = visaTypeWindowSelector.concat("//span[text() = '" + expectedVisaTypeValue + "']/..");
        click.leftClick(specificVisaTypeSelector, "Specific visa type select");
    }
    ...
    private void selectSpecificProcessingSpeed(PhpTravelsModel phpTravelsModel) {
        ...
        String specificVisaTypeSelector = processingSpeedWindowSelector.concat("//div[text() = '" + expectedProcessingSpeedValue + "']/ancestor::div[@class = 'input-dropdown-item']");
        click.leftClick(specificVisaTypeSelector, "Processing speed type select");
    }
    ...
    private void checkVisaTypeValueBeforeChange() {
        ...
        checkSelectValue(visaTypeValueSelector, "visa type", expectedVisaTypeValue);
    }
    ...
    private void checkProcessingSpeedValueBeforeChange() {
        ...
        checkSelectValue(processingSpeedValueSelector, "processing speed", expectedProcessingSpeedValue);
    }

    private void checkTravellersValueBeforeChange() {
        String expectedTravellersValue = "1 Traveler";
        checkSelectValue(travellersValueSelector, "travelers", expectedTravellersValue);
    }
    ...
    private void checkTravellersValueAfterChange(PhpTravelsModel phpTravelsModel) {
        ...
        checkSelectValue(travellersValueSelector, "travellers", expectedTravellersValue.concat(" Travelers"));
    }
    ...
    private void checkIfDatePickerWindowIsDisplayed(boolean shouldBeDisplayed) {
        checkIfWindowIsDisplayed(datePickerWindowSelector, "Date picker", shouldBeDisplayed);
    }

    private void checkIfVisaTypeWindowIsDisplayed(boolean shouldBeDisplayed) {
        checkIfWindowIsDisplayed(visaTypeWindowSelector, "Date picker", shouldBeDisplayed);
    }

    private void checkIfProcessingSpeedWindowIsDisplayed(boolean shouldBeDisplayed) {
        checkIfWindowIsDisplayed(processingSpeedWindowSelector, "Date picker", shouldBeDisplayed);
    }

    private void checkIfTravellersWindowIsDisplayed(boolean shouldBeDisplayed) {
        checkIfWindowIsDisplayed(travellersWindowSelector, "Date picker", shouldBeDisplayed);
    }

    private void checkIfWindowIsDisplayed(String windowSelector, String selectorName, boolean shouldBeDisplayed) {
        if (shouldBeDisplayed) {
            check.isElementPresent(windowSelector, selectorName + " window");
            check.isVisible(windowSelector, selectorName + " window");
            log.info("{} window has been displayed.", selectorName);
        } else {
            check.isElementNotPresent(windowSelector, selectorName + " window");
            log.info("{} window has been closed.", selectorName);
        }
    }
    ...
    private void checkArrivalCountryBeforeInput() {
        String expectedSelectedCountry = arrivalSpanSelector.concat("//span[@x-text = 'getSelectedName()']");
        compareCountryBeforeOrAfterInput("Select Country", expectedSelectedCountry, "Arrival", "before");
    }

    private void checkDepartureCountryBeforeInput() {
        String expectedSelectedCountry = departureSpanSelector.concat("//span[@x-text = 'getSelectedName()']");
        compareCountryBeforeOrAfterInput("Select Country", expectedSelectedCountry, "Departure", "before");
    }
    ...
    private void checkDepartureCountryAfterInput(PhpTravelsModel phpTravelsModel) {
        ...
        String expectedSelectedCountry = departureSpanSelector.concat("//span[@x-text = 'getSelectedName()']");
        compareCountryBeforeOrAfterInput(expectedCountry, expectedSelectedCountry, "Departure", "after");
    }
    ...
    private void checkArrivalCountryAfterInput(PhpTravelsModel phpTravelsModel) {
        ...
        String expectedSelectedCountry = arrivalSpanSelector.concat("//span[@x-text = 'getSelectedName()']");
        compareCountryBeforeOrAfterInput(expectedCity, expectedSelectedCountry, "Arrival", "after");
    }

    private void compareCountryBeforeOrAfterInput(String expectedLocationValue, String elementSelector, String destination, String inputStage) {
        check.hasText(elementSelector, expectedLocationValue, destination + " country check " + inputStage + " input");
        log.info("{} country {} input value meets expected value.", destination, inputStage);
    }

    private void clickOnDepartureCountryInput() {
        clickOnLocationInput(departureSpanSelector, "Departure country");
    }

    private void clickOnArrivalCountryInput() {
        clickOnLocationInput(arrivalSpanSelector, "Arrival country");
    }
    ...
    private void checkIfCountriesAreDisplayed() {
        checkIfCountryIsDisplayed(countriesContainerSelector, countrySelector);
    }

    private void checkIfCountryIsDisplayed(String containerSelector, String selector) {
        check.isVisible(containerSelector, "Country container");
        check.isElementPresent(selector, "Selector");
    }
    ...
    private void findAndSelectDepartureCountry(PhpTravelsModel phpTravelsModel) {
        ...
        typeCountryToCountryInput(departureCountryInputSelector, expectedDestinationCountryName, "Departure country");

        String countryButtonSelector = "//label/text()[normalize-space(.) = 'From Country']/ancestor::div[@class = 'form-control']//div[@class = 'input-dropdown-content show']//span[text() = '" + expectedDestinationCountryName + "']";
        clickOnCountryButton(expectedDestinationCountryName, countryButtonSelector);
    }
    ...
    private void findAndSelectArrivalCountry(PhpTravelsModel phpTravelsModel) {
        ...
        typeCountryToCountryInput(arrivalCountryInputSelector, expectedDestinationCountryName, "Arrival country");

        String countryButtonSelector = "//label/text()[normalize-space(.) = 'To Country']/ancestor::div[@class = 'form-control']//div[@class = 'input-dropdown-content show']//span[text() = '" + expectedDestinationCountryName + "']";
        clickOnCountryButton(expectedDestinationCountryName, countryButtonSelector);
    }

    private void typeCountryToCountryInput(String countryInputSelector, String expectedCountry, String inputType) {
        check.isEnabled(countryInputSelector, "Country input");
        send.sendKeysToElementWithNoLeave(countryInputSelector, expectedCountry, "Country input");
        log.info("{} has been typed to {} input.", expectedCountry, inputType);
    }

    private void clickOnCountryButton(String expectedCountry, String countryButtonSelector) {
        check.isElementPresent(countryButtonSelector, "Country button");
        click.leftClick(countryButtonSelector, "Country button");

        log.info("{} has been selected.", expectedCountry);
    }

    private void checkIfTabIsActive() {
        check.containsAttribute(visaTabSelector, "class", "text-primary", "Visa tab activity");
        log.info("Visa tab is an active tab.");
    }

    private void checkIfSearchBarIsDisplayed() {
        check.isVisible(visaTabSelector, "Search bar");
